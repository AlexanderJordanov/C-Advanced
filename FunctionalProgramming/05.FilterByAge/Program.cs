...
void PrintPeople(Person[] people, Func<Person, bool> predicate, Func<Person, string> formatter)
{
...
Func<Person, bool> GetAgeCondition(string? filter, int filterAge)
{
    if (filter == "younger")
    {
...
Func<Person, string> GetFormatter(string? format)
{
    if (format == "name")
    {
        return p => $"{p.Name}";
